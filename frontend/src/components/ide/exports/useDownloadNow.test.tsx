...
describe('useDownloadNow', () => {
  ...
  it('streams the response into a named download and resets progress', async () => {
    mocks.downloadExport.mockResolvedValue(new Response('id,name\n1,Ada\n', {
    ...
    expect(result.current).toEqual(expect.objectContaining({ isDownloading: false, bytesDownloaded: 0 }))
  })

  it('aborts an active export and reports cancellation', async () => {
    mocks.downloadExport.mockImplementation((
      _org: string, _workspace: number, _connection: number, _session: string,
    ...
    act(() => { download = result.current.download(7, 'select 1') })
