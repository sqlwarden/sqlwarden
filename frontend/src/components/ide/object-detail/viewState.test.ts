...
describe('resolveObjectViewState', () => {
  it('no session -> no-session (even mid-load)', () => {
    expect(
      resolveObjectViewState({ hasSession: false, isLoading: true, error: null, hasData: false }),
    ).toEqual({ kind: 'no-session' })
  })
  it('loading with session and no data -> loading', () => {
    expect(
      resolveObjectViewState({ hasSession: true, isLoading: true, error: null, hasData: false }),
  ...
  it('data present -> ready (even while refetching)', () => {
    expect(
      resolveObjectViewState({ hasSession: true, isLoading: true, error: null, hasData: true }),
    ).toEqual({ kind: 'ready' })
  })
  it('no session wins over cached data (expired cloud session shows reconnect)', () => {
    expect(
      resolveObjectViewState({ hasSession: false, isLoading: false, error: null, hasData: true }),
    ).toEqual({ kind: 'no-session' })
  })
})
