...
const ref: ObjectRef = { scope: [{ kind: 'schema', name: 'public' }], kind: 'table', name: 'users' }
...
describe('objectTab', () => {
  it('builds a stable id from connection + ref', () => {
    expect(objectTabId(7, ref)).toBe('object:7:schema=public:table:users')
  ...
  it('creates an object tab carrying ref + connection + driver', () => {
    ...
    expect(tab.id).toBe('object:7:schema=public:table:users')
