...
function dependencies(overrides: Partial<QueryExecutionDependencies> = {}): QueryExecutionDependencies {
  return {
...
describe('executeQuery', () => {
  ...
  it('records cancellation and always releases transient execution state', async () => {
    ...
    const deps = dependencies({ ensureSession: vi.fn(async () => { throw error }) })

    await executeQuery({ tabId: 'tab-1', connectionId: 7, sql: 'select 1', controller: new AbortController() }, deps)

    expect(deps.setResult).toHaveBeenLastCalledWith('tab-1', { status: 'cancelled', sql: 'select 1' })
    expect(deps.setController).toHaveBeenLastCalledWith('tab-1', null)
    expect(deps.setRunning).toHaveBeenLastCalledWith('tab-1', false)
  })

  it('normalizes unknown failures without rejecting the UI event', async () => {
    const deps = dependencies({ ensureSession: vi.fn(async () => { throw { reason: 'unknown' } }) })
