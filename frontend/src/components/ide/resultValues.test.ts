...
describe('result selection', () => {
  const selection: CellSelection = { anchor: { rowIdx: 3, colIdx: 4 }, active: { rowIdx: 1, colIdx: 2 } }
...
describe('formatResultValue', () => {
  const cases: Array<[ResultValue, { display: string; isNull: boolean; isNumeric: boolean }]> = [
    ...
    [{ type: 'text', text: 'hello' }, { display: 'hello', isNull: false, isNumeric: false }],
    [{ type: 'integer', integer: 42 }, { display: '42', isNull: false, isNumeric: true }],
    [{ type: 'float', float: 1.5 }, { display: '1.5', isNull: false, isNumeric: true }],
    [{ type: 'decimal', decimal: '9.99' }, { display: '9.99', isNull: false, isNumeric: true }],
    [{ type: 'bool', bool: false }, { display: 'false', isNull: false, isNumeric: false }],
    [{ type: 'time', time: '2026-01-01' }, { display: '2026-01-01', isNull: false, isNumeric: false }],
    [{ type: 'bytes', bytes: [0] }, { display: '(binary)', isNull: false, isNumeric: false }],
  ]
