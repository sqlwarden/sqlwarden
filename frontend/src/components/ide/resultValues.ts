...
export function cellInRange(rowIndex: number, columnIndex: number, selection: CellSelection | null): boolean {
...
export function formatResultValue(value: ResultValue): { display: string; isNull: boolean; isNumeric: boolean } {
  if (value.type === 'null') return { display: 'NULL', isNull: true, isNumeric: false }
  switch (value.type) {
    case 'text': return { display: value.text ?? '', isNull: false, isNumeric: false }
    case 'integer': return { display: String(value.integer ?? 0), isNull: false, isNumeric: true }
    case 'float': return { display: String(value.float ?? 0), isNull: false, isNumeric: true }
    case 'decimal': return { display: value.decimal ?? '', isNull: false, isNumeric: true }
    case 'bool': return { display: value.bool ? 'true' : 'false', isNull: false, isNumeric: false }
    case 'time': return { display: value.time ?? '', isNull: false, isNumeric: false }
