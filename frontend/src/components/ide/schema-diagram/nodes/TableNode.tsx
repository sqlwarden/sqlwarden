...
export function TableNode({ id, data }: NodeProps & { data: TableNodeData }) {
  ...
  const visibleColumns = data.keysOnly
    ? data.columns.filter((c) => isKeyColumn(c.name, data))
    : data.columns
  ...
  const handlesSignature = data.collapsed
    ...
    : visibleColumns
        .map(
          (column) =>
            `${column.name}:${incomingByCol[column.name] ? 'in' : ''}:${outgoingByCol[column.name] ? 'out' : ''}`,
        )
        .join('|')
  ...
  return (
    <div
      className="group relative cursor-move rounded border border-border bg-card text-xs shadow-sm"
      style={{ width: '100%' }}
    >
      ...
      {!data.collapsed && (
        <div>
          {data.loading && (
            <div className="px-2 py-1 text-[10px] text-muted-foreground">Loading columns…</div>
          )}
          ...
          {visibleColumns.map((c) => {
            ...
            const rel: HoverRelation | null =
              out && !out.hidden ? { source: id, target: refKey(out.target), column: c.name } : null
            return (
              <div
                key={c.name}
                className={cn(
                  'relative flex items-center gap-1 border-b border-border/50 px-2 py-0.5 last:border-b-0',
                  rel && 'hover:bg-primary/5',
                )}
              ...
              >
                ...
                {inc?.hidden && (
                  <Tip label="Add tables that reference this column" side="left">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        data.onExpand(inc.sources, { fromKey: id, direction: 'in' })
                      }}
                ...
                <span
                  className={cn(
                    ...
                    data.pk.has(c.name) ? 'font-semibold text-foreground' : 'text-muted-foreground',
                  )}
                >
                  {c.name}
                </span>
                ...
                <span
                  title={c.data_type}
                  className="ml-1 max-w-[50%] shrink truncate font-mono text-[10px] text-muted-foreground"
                >
                  {c.data_type}
                </span>
                {data.pk.has(c.name) && (
                  <span className="shrink-0 text-[9px] font-medium text-chart-4">PK</span>
                )}
                ...
                {out?.hidden && (
                  <Tip label={`Add referenced table "${out.target.name}"`} side="right">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        data.onExpand([out.target], { fromKey: id, direction: 'out' })
                      }}
