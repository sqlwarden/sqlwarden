...
function WorkspaceRolesPage({ orgSlug, workspaceId }: { orgSlug: string; workspaceId: string }) {
  ...
  const canModifyRoles = hasPermission(
              ...
              >
                ...
                <DialogContent className="sm:max-w-2xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Define a permission set that can be assigned via workspace policies.
                    </DialogDescription>
                  </DialogHeader>
                  <form className="mt-6 flex flex-col gap-6" onSubmit={submitCreateRole}>
                    ...
                    <div className="flex flex-col gap-2">
                      <Label>Scope</Label>
                      <Select
                        items={workspaceRoleScopes.map((scope) => ({
                          label: roleScopeLabel(scope),
                          value: scope,
                        }))}
                        value={scopeType}
                        onValueChange={(value) => {
                      ...
                      >
                        <SelectTrigger
                          className="w-full"
                          aria-invalid={fieldErrors.scope_type ? true : undefined}
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      {fieldErrors.scope_type ? (
                        <p className="text-sm text-destructive">{fieldErrors.scope_type}</p>
                      ) : null}
                    </div>

                    <div className="flex flex-col gap-2">
                      <Label htmlFor="workspace-role-description">
                        Description{' '}
            ...
            <Table>
              ...
              <TableBody>
                {effectivePermissions.isLoading || roles.isLoading ? (
                  <RolesTableSkeleton showActions={canModifyRoles} />
                ) : null}
                {roles.isError ? (
                  <TableEmptyState
                    colSpan={canModifyRoles ? 5 : 4}
                    icon="user-shield-01"
