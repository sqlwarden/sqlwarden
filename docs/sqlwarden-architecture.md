...
- Public compile-time composition root and typed distribution contracts for separate open-core builds.
...
├── app/                              # public application composition root
├── authorization/                    # public authorization contracts
├── buildinfo/                        # core/distribution build identity
├── distribution/                     # public compile-time distribution contracts
├── docs/
│   ├── distribution-architecture.md  # open-core composition boundary
...
│       ├── styles.css
│       ├── routes/
│       ├── distribution/             # build-time providers, routes, and navigation
│       ├── components/
│       └── lib/
├── internal/
...
`cmd/api` should remain thin and construct the public `app` package. New reusable HTTP behavior belongs in `internal/web`, not in `cmd/api`, so future `cmd/desktop` and paid distributions can embed or start the same web application.
...
SQLWarden uses compile-time dependency injection for open-core distributions. The separate enterprise repository imports Community's public `app`, `distribution`, `authorization`, and `buildinfo` packages, constructs paid implementations, and supplies one dependency object to the application composition root. Community never imports enterprise code. This is a trusted, lockstep source integration rather than a runtime plugin system or a versioned third-party extension SDK.

The injected surface currently supports:

- Restrictive authorization constraints layered after mandatory Community RBAC.
- Application-local permission metadata and additive builtin-role defaults.
- Separate migration streams and ledgers in the shared application database.
- Persisted background job definitions and event reporting.
- Product-native Chi routes mounted inside existing auth/resource contexts.
- Account, organization, session, request-context, logging, and job services.
- Distribution lifecycle startup and shutdown.
- Distribution-owned embedded frontend assets and build identity.
- Frontend providers, TanStack routes, and app-shell navigation composed at build time.

Community is the default implementation for every seam. Injected authorization may further restrict a Community-approved request but cannot grant a request denied by Community RBAC. New seams should be narrow typed interfaces introduced only when a concrete paid feature needs them; do not build a generic hook/event/module framework for hypothetical extensions.

See `docs/distribution-architecture.md` for the composition contract, startup order, frontend build model, and rules for introducing future seams.

Likely enterprise-only features:

- SAML/LDAP and advanced enterprise identity integrations. Standards-based OIDC remains part of the Community product direction.
- SCIM provisioning.
- Tamper-evident audit logs.
- SIEM forwarding.
