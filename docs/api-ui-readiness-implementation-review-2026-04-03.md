...
- unified org members, teams, workspaces, and environments on the same paginated `{items,page,page_size,total}` list contract
- added search, pagination, sorting, and concrete flat filters across org members, teams, workspaces, and environments
...
- updated repository context guidance so future UI-facing list resources follow the same shared contract
...
- org members, teams, workspaces, environments, connections, and workspace policies use paginated `{items,page,page_size,total}` responses
- resource-specific filters stay flat query params: `role`, `slug`, `name`, and existing endpoint-specific filters
