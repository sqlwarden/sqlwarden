...
func postgresStatementObject(kind string) metadata.Object {
...
func TestPostgresGenerateStatement(t *testing.T) {
	...
	want := "INSERT INTO \"public\".\"generated\"\"orders\" (\n  \"id\",\n  \"order\"\"note\"\n)\nVALUES (\n  $1,\n  $2\n);"
	if generated != want {
	...
	view := object
	view.Ref.Kind = "view"
	if _, err := driver.Generate(statement.Request{Operation: statement.OperationUpdate, Object: view}); err == nil {
		t.Fatal("expected UPDATE generation for a view to fail")
	}
}

func TestPostgresGeneratedStatementsExecuteSafely(t *testing.T) {
	driver := newConnectedDriver(t)
	ctx := context.Background()
	object := postgresStatementObject("table")
	qualified := postgresDDLQualified("public", object.Ref.Name)
