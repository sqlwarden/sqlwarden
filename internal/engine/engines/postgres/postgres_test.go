...
func mustExec(t *testing.T, d engine.Driver, sql string) {
	t.Helper()
...
func TestPostgresViewDefinitionAndComments(t *testing.T) {
	...
	tbl, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "table", Name: "defs_t"}})
	if err != nil {
	...
	view, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "view", Name: "defs_v"}})
	if err != nil {
		t.Fatalf("InspectObjects view: %v", err)
	}
	if def := descriptorByTitle(view[0].Descriptors, "Definition"); def != nil {
		t.Fatalf("bulk InspectObjects must not inline the view definition: %+v", view[0].Descriptors)
	}

	def, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: pgTestScope("public"), Kind: "view", Name: "defs_v"})
	if err != nil {
		t.Fatalf("InspectDefinition view: %v", err)
	}
	if def == nil || def.Kind != "source" || def.Title != "Definition" || !strings.Contains(strings.ToUpper(def.Source.Body), "SELECT") {
		t.Fatalf("lazy view definition missing/blank: %+v", def)
	}
}
...
func TestPostgresTableDDL(t *testing.T) {
	...
	objs, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "table", Name: "ddl_child"}})
	if err != nil {
		t.Fatalf("InspectObjects: %v", err)
	}
	if descriptorByTitle(objs[0].Descriptors, "DDL") != nil {
		t.Fatalf("bulk InspectObjects must not inline table DDL: %+v", objs[0].Descriptors)
	}

	ddl, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: pgTestScope("public"), Kind: "table", Name: "ddl_child"})
	if err != nil {
		t.Fatalf("InspectDefinition: %v", err)
	}
	if ddl == nil || ddl.Kind != "source" || ddl.Title != "DDL" {
		t.Fatalf("missing DDL descriptor: %+v", ddl)
	}
	for _, want := range []string{
		...
		if !strings.Contains(ddl.Source.Body, want) {
			t.Fatalf("DDL missing %q:\n%s", want, ddl.Source.Body)
		}
	}
}
...
func TestPostgresTableDDLRoundTrips(t *testing.T) {
	...
	parentRef := metadata.ObjectRef{Scope: pgTestScope("ddl_rt"), Kind: "table", Name: "parent"}
	childRef := metadata.ObjectRef{Scope: pgTestScope("ddl_rt"), Kind: "table", Name: "child"}

	genDDL := func(ref metadata.ObjectRef) string {
		src, err := d.InspectDefinition(ctx, ref)
		if err != nil {
			t.Fatalf("InspectDefinition %s: %v", ref.Name, err)
		}
		if src == nil {
			t.Fatalf("missing DDL for %s", ref.Name)
		}
...
func TestPostgresIndexColumns(t *testing.T) {
	...
	objs, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "table", Name: "idx_cols"}})
	if err != nil {
		t.Fatalf("InspectObjects: %v", err)
	}
	var found *metadata.SecondaryIndex
...
func TestPostgresInspectObjectsHonorsFilter(t *testing.T) {
	...
	objs, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "table", Name: "intro_a"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresInspectObjectsFunctionDescriptors(t *testing.T) {
	...
	objs, err := d.InspectObjects(ctx, []metadata.ObjectRef{{Scope: pgTestScope("public"), Kind: "function", Name: "intro_add"}})
	if err != nil {
		t.Fatal(err)
	}
	if len(objs) != 1 {
	...
	if !hasFields {
		t.Errorf("function should expose a fields descriptor, got %+v", o.Descriptors)
	}
	if hasSource {
		t.Errorf("bulk InspectObjects must not inline the function definition: %+v", o.Descriptors)
	}

	def, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: pgTestScope("public"), Kind: "function", Name: "intro_add"})
	if err != nil {
		t.Fatalf("InspectDefinition: %v", err)
	}
	if def == nil || def.Kind != "source" || def.Title != "Definition" ||
		!strings.Contains(strings.ToUpper(def.Source.Body), "CREATE") {
		t.Fatalf("lazy function definition missing/blank: %+v", def)
	}
}
...
func TestPostgresInspectDefinition(t *testing.T) {
	...
	tbl, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: scope, Kind: "table", Name: "idef_t"})
	if err != nil {
		t.Fatalf("InspectDefinition(table): %v", err)
	}
	if tbl == nil || tbl.Title != "DDL" || !strings.Contains(tbl.Source.Body, "CREATE TABLE") {
		t.Fatalf("table definition = %+v", tbl)
	}

	view, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: scope, Kind: "view", Name: "idef_v"})
	if err != nil {
		t.Fatalf("InspectDefinition(view): %v", err)
	}
	if view == nil || view.Title != "Definition" || !strings.Contains(strings.ToUpper(view.Source.Body), "SELECT") {
		t.Fatalf("view definition = %+v", view)
	}

	fn, err := d.InspectDefinition(ctx, metadata.ObjectRef{Scope: scope, Kind: "function", Name: "idef_fn"})
	if err != nil {
		t.Fatalf("InspectDefinition(function): %v", err)
	}
	if fn == nil || fn.Title != "Definition" || !strings.Contains(strings.ToUpper(fn.Source.Body), "CREATE") {
...
func pgTestScope(namespace string) metadata.ScopePath {
	return metadata.NewScopePath(metadata.ScopeSegment{Kind: "schema", Name: namespace})
}
