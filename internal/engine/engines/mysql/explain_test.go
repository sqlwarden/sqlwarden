...
func TestMySQLExplain(t *testing.T) {
	...
	if want := (explain.Plan{Statement: "EXPLAIN SELECT 1"}); !reflect.DeepEqual(plain, want) {
	...
	if want := (explain.Plan{Statement: "EXPLAIN ANALYZE SELECT 1"}); !reflect.DeepEqual(analyze, want) {
