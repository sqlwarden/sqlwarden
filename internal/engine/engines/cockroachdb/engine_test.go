package cockroachdb

import (
	"testing"

	"github.com/sqlwarden/internal/engine"
	"github.com/sqlwarden/internal/engine/enginetest"
)

func TestCockroachDBEngineContract(t *testing.T) {
	enginetest.RunCapabilityContract(t, "cockroachdb")
	set, ok := engine.Describe("cockroachdb")
	if !ok {
		t.Fatal("cockroachdb engine not registered")
	}
	for _, capability := range []engine.Capability{
		engine.CapabilitySQLParse,
		engine.CapabilitySQLClassify,
		engine.CapabilitySQLComplete,
	} {
		if !set.Capabilities[capability] {
			t.Errorf("%s must be true", capability)
		}
	}
	for _, capability := range []engine.Capability{
		engine.CapabilitySQLRewrite,
	} {
		if set.Capabilities[capability] {
			t.Errorf("%s must remain false", capability)
		}
	}
	enginetest.RunConnectionContract(t, "cockroachdb", engine.ConnectionConfig{DSN: testDSN, Driver: "cockroachdb"})
}
