...
func Register(reg Registration) {
	if reg.ID == "" {
		panic("engine: Register called with empty id")
	}
	if reg.New == nil {
		panic(fmt.Sprintf("engine: Register %q called with nil New", reg.ID))
	}
	registryMu.Lock()
	defer registryMu.Unlock()
	if _, exists := registry[reg.ID]; exists {
		panic(fmt.Sprintf("engine: Register called twice for engine %q", reg.ID))
	}
	registry[reg.ID] = reg
}
