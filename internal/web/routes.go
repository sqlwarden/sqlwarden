...
func (app *application) routes() http.Handler {
	...
	mux.Route("/api/v1", func(r chi.Router) {
		...
		r.With(app.requireServerMode, app.requireAccount, app.requireInstanceAdmin).Post("/orgs", app.createOrg)
		...
		r.Group(func(r chi.Router) {
			...
			r.With(app.requireServerMode).Patch("/account", app.updateAccount)
			r.With(app.requireServerMode).Patch("/account/password", app.updateAccountPassword)
			...
			r.Route("/workspaces", func(r chi.Router) {
				...
				r.Route("/{ws_id}", func(r chi.Router) {
					...
					r.Route("/files/private", func(r chi.Router) {
						...
						r.Get("/search", app.searchPrivateWorkspaceFiles)
						...
						r.Route("/{env_id}", func(r chi.Router) {
							...
							r.Route("/connections", func(r chi.Router) {
								...
								r.Route("/{conn_id}", func(r chi.Router) {
									...
									r.Get("/schema/snapshot", app.getConnectionSchemaSnapshot)
									r.Get("/schema/directory", app.getConnectionSchemaDirectory)
									r.Get("/schema/completion-index", app.getConnectionCompletionIndex)
									r.Post("/schema/objects", app.getConnectionSchemaObjects)
									r.Get("/schema/object/definition", app.getConnectionSchemaObjectDefinition)
									r.Get("/schema/relationships", app.getConnectionSchemaRelationships)
									r.Post("/schema/refresh", app.refreshConnectionSchema)
									r.Post("/schema/mutations", app.applyConnectionDDL)
									r.Post("/schema/statements", app.generateConnectionStatement)
								})
							})
						})
					})
		...
		r.Route("/orgs/{org_slug}", func(r chi.Router) {
			...
			r.With(app.requireServerMode, app.requireOrgPermission("org:delete")).Delete("/", app.deleteOrg)
			...
			r.Route("/workspaces", func(r chi.Router) {
				...
				r.Route("/{ws_id}", func(r chi.Router) {
					...
					r.Route("/files/shared", func(r chi.Router) {
						...
						r.Get("/search", app.searchSharedWorkspaceFiles)
						r.Route("/{file_id}", func(r chi.Router) {
						...
						r.Route("/{env_id}", func(r chi.Router) {
							...
							r.Route("/connections", func(r chi.Router) {
								...
								r.Route("/{conn_id}", func(r chi.Router) {
									r.Use(app.connCtx)
									r.Get("/", app.getConnection)
									r.With(app.requireConnectionPermission("conn:update")).Get("/dsn", app.getConnectionDSN)
									r.With(app.requireConnectionPermission("conn:update")).Get("/tls", app.getConnectionTLS)
									r.With(app.requireConnectionPermission("conn:update")).Get("/ssh", app.getConnectionSSH)
									r.With(app.requireConnectionPermission("conn:update")).Delete("/tls", app.deleteConnectionTLS)
									r.With(app.requireConnectionPermission("conn:update")).Delete("/ssh", app.deleteConnectionSSH)
